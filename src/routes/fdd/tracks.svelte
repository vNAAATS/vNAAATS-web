...
<div class="flex flex-col w-full h-1/3 border-2 border-t-blue-0 border-l-blue-0 font-bitmap text-xl font-thin">
    ...
    <div class="overflow-y-scroll h-full">
        ...
        {#each westbound as track}
            <TrackStrip id={track.Identifier} route={track.Route} flightLevels={track.FlightLevels} 
            direction={track.Direction} validFrom={track.ValidFrom} validTo={track.ValidTo} />
        {/each}
        <div class="flex flex-row justify-center w-full bg-grey-500 bg-opacity-70 select-none hover:cursor-default">
            Eastbound
        </div>
