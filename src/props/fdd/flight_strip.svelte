...
<div
  class="hover:border-green-500 hover:border-2"
  on:click={() => dataHandler.setAsel(callsign)}
>
  {#if track === "RR"}
    <div
      class="flex flex-col items-center w-full h-14 {darkerColour} {isActive
        ? 'bg-opacity-75'
    ...
    >
        ...
        <div class="flex flex-row w-0.68">
          {#each route as rte}
            <div class="w-1/11 select-none hover:cursor-default">
              {utils.coordToArray(rte).length != 0
                ? utils.coordToArray(rte)[0]
                : rte}
            </div>
          {/each}
        </div>
      ...
      <div
        class="flex flex-row justify-start items-center w-full select-none hover:cursor-default"
      >
        <div class="flex flex-row w-0.32" />
        <div class="flex flex-row w-0.68">
          {#each route as rte}
  ...
  {/if}
  <div
    class="flex flex-row items-center w-full h-8 {colour} {isActive
      ? 'bg-opacity-75'
      : ''} text-black hover:cursor-default"
    on:mousedown={stripMouseDown}
    on:mouseup={stripMouseUp}
  ...
  >
    ...
    <div
      class="flex flex-row justify-end w-1/5 hover:cursor-default select-none"
    >
